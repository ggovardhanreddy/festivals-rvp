...
export const onRequest = async ({ request, env }: FunctionContext) => {
  ...
  if (url.pathname.endsWith("/login") && request.method === "POST") {
    ...
    if (!(await passwordMatches(payload.password ?? "", env.ADMIN_PASSWORD_HASH)))
    ...
    return new Response(JSON.stringify({ ok: true }), {
      headers: {
        ...
        "set-cookie": `rvp_admin=${value}.${await sign(value, env.ADMIN_SESSION_SECRET)}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=86400`,
      },
    });
  }
  return new Response(
    JSON.stringify({ error: "Production archive writes require a local Git workflow." }),
